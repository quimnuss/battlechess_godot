...
@onready var v_scroll_bar = $"."
@onready var game_list_v_box_container = %GameListVBoxContainer
...
func _input(ev) -> void:
    ...
    if ev is InputEventMouseButton:
        if ev.pressed and get_global_rect().has_point(ev.global_position):
            prints("looking for swipe", ev.global_position)
            look_for_swipe = true
            ...
            if flick_dur > 0.0:
                ...
                prints("tween scroll from", source, "->", target)
    ...
    if ev is InputEventMouseMotion:
        ...
        if swiping:
            ...
            var max_v_scroll = max(0, game_list_v_box_container.get_rect().size.y - v_scroll_bar.get_rect().size.y)
            var dy: int = swipe_start.y - delta.y
            if dy <= 0:
                swipe_mouse_start.y = ev.global_position.y
                swipe_start.y = 0
                set_v_scroll(0)
                return
            elif dy > max_v_scroll:
                swipe_start.y = max_v_scroll
                delta.y = 0
                swipe_mouse_start.y = ev.global_position.y
                set_v_scroll(max_v_scroll)
                return

            set_h_scroll(swipe_start.x - delta.x)
            prints("setting scroll to", swipe_start.y - delta.y)
